...
"""
Code taken from https://github.com/prov-gigapath/prov-gigapath. Adapted LongNetViT to better fit the ModalTune pipeline
"""
...
from .torchscale.component.dilated_attention import DilatedAttentionLoraAdapter
...
class LongNetViT(nn.Module):
    ...
    def initialize_vit_weights(self):
        ...
        self.apply(self._init_lora_weights)
    ...
    def _init_lora_weights(self, m):
        if isinstance(m, DilatedAttentionLoraAdapter):
            m.reset_parameters()
    ...
    def forward(self, x, coords, all_layer_embed=False):
        ...
        if all_layer_embed:
            x_list = self.encoder.layer_forward(
                x=x,
        ...
        if self.return_feats:
            return outcomes, x_list[-1]
        else:
            return outcomes

    def load_slide_encoder(self, pretrained=False, weights_location="./"):
        if not pretrained:
            return None

        local_path = os.path.join(weights_location, "slide_encoder.pth")
