...
class DilatedAttentionLoraAdapter(DilatedAttention):
    ...
    def __init__(
        ...
        self.img_lora_dim = args.img_lora_dim
        self.mm_lora_dim = args.mm_lora_dim
        ...
        self.img_lora_scaling = self.lora_alpha / self.img_lora_dim
        self.mm_lora_scaling = self.lora_alpha / self.mm_lora_dim

        self.q_proj_lora_dict = nn.ModuleDict(
            {
                "lora_A_img": nn.Linear(embed_dim, self.img_lora_dim, bias=False),
                "lora_B_img": nn.Linear(self.img_lora_dim, embed_dim, bias=False),
                "lora_A_gene": nn.Linear(embed_dim, self.mm_lora_dim, bias=False),
                "lora_B_gene": nn.Linear(self.mm_lora_dim, embed_dim, bias=False),
                "lora_A_task": nn.Linear(embed_dim, self.mm_lora_dim, bias=False),
                "lora_B_task": nn.Linear(self.mm_lora_dim, embed_dim, bias=False),
            }
        )
        self.k_proj_lora_dict = nn.ModuleDict(
            {
                "lora_A_img": nn.Linear(embed_dim, self.img_lora_dim, bias=False),
                "lora_B_img": nn.Linear(self.img_lora_dim, embed_dim, bias=False),
                "lora_A_gene": nn.Linear(embed_dim, self.mm_lora_dim, bias=False),
        ...
        self.v_proj_lora_dict = nn.ModuleDict(
            {
                "lora_A_img": nn.Linear(embed_dim, self.img_lora_dim, bias=False),
                "lora_B_img": nn.Linear(self.img_lora_dim, embed_dim, bias=False),
                "lora_A_gene": nn.Linear(embed_dim, self.mm_lora_dim, bias=False),
    ...
    def reset_parameters(self):
        ...
        for k, v in self.q_proj_dict.items():
            if "A" in k:
                nn.init.kaiming_uniform_(v.weight, a=math.sqrt(5))
            elif "B" in k:
                nn.init.zeros_(v.weight)
        for k, v in self.k_proj_dict.items():
            if "A" in k:
                nn.init.kaiming_uniform_(v.weight, a=math.sqrt(5))
            elif "B" in k:
                nn.init.zeros_(v.weight)
        for k, v in self.v_proj_dict.items():
            if "A" in k:
                nn.init.kaiming_uniform_(v.weight, a=math.sqrt(5))
            elif "B" in k:
                nn.init.zeros_(v.weight)
    ...
    def forward(
        ...
        def apply_lora(input, gene, task, proj, proj_dict):
            result = proj(input)
            # 1, N, D
            img_delta = (
                proj_dict["lora_B_img"](
                    proj_dict["lora_A_img"](self.lora_dropout(input))
                )
                * self.img_lora_scaling
            )
            # 1, 1, D
            gene_delta = (
                proj_dict["lora_B_gene"](
                    proj_dict["lora_A_gene"](self.lora_dropout(gene))
                )
                * self.mm_lora_scaling
            )
            # 1, 1, D
            task_delta = (
                proj_dict["lora_B_task"](
                    proj_dict["lora_A_task"](self.lora_dropout(task))
                )
                * self.mm_lora_scaling
            )
            # add with broadcasting
            result = result + (img_delta + gene_delta + task_delta) / 3
            return result

        q = apply_lora(query, gene, task, self.q_proj, self.q_proj_lora_dict)
        k = apply_lora(key, gene, task, self.k_proj, self.k_proj_lora_dict)
        v = apply_lora(value, gene, task, self.v_proj, self.v_proj_lora_dict)
